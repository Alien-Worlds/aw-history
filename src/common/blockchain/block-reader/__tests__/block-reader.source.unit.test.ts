...
describe('BlockReaderSource Unit tests', () => {
  it('"updateConnectionState" should return a handler assigned to the connection state', () => {
    const source = new BlockReaderSource({} as any);
    const handler = jest.fn();
    source.addConnectionStateHandler(BlockReaderConnectionState.Connected, handler);

    (source as any).updateConnectionState(BlockReaderConnectionState.Connected);
  ...
  it('"addConnectionStateHandler" should log warning if there is already a handler assigned to the state', () => {
    ...
    source.addConnectionStateHandler(BlockReaderConnectionState.Connected, jest.fn());
    source.addConnectionStateHandler(BlockReaderConnectionState.Connected, jest.fn());
  ...
  it('"addConnectionStateHandler" should assign a handler to the state', () => {
    ...
    expect(
      (source as any).connectionChangeHandlers.has(BlockReaderConnectionState.Connected)
    ).toBeTruthy();
  });

  it('"isConnected" should return true if source is connected', () => {
    const source = new BlockReaderSource({} as any);
    (source as any).connectionState = BlockReaderConnectionState.Connected;
  ...
  it('"isConnected" should return false if source is not connected', () => {
    const source = new BlockReaderSource({} as any);
    (source as any).connectionState = BlockReaderConnectionState.Idle;
