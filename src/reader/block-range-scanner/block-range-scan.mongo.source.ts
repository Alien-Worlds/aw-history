...
export class BlockRangeScanMongoSource extends MongoCollectionSource<BlockRangeScanDocument> {
  ...
  public async startNextScan(scanKey: string): Promise<BlockRangeScanDocument> {
    ...
    return result.value as BlockRangeScanDocument;
  }
  ...
  public async findCompletedParentNode(document: BlockRangeScanDocument) {
    ...
    if (parent_id) {
      ...
      if (matchingParentCount == 0) {
        const parentDocument: BlockRangeScanDocument = await this.collection.findOne({
          _id: parent_id,
        });
  ...
  public async updateProcessedBlockNumber(
    ...
    const range: BlockRangeScanDocument = await this.findRangeForBlockNumber(
      blockNumber,
      scanKey
    );
